...
private String audioUrl;
	...
	private int TIMEPLUSVIDEO = 1000;
...
//		}
		...
		MyLog.d(TAG, "===loadLyricSong start0==id=" + id);
		MyLog.d(" ", " ");MyLog.d(" ", " ");MyLog.d(" ", " ");
...
//			}
		...
		if (cursor.moveToFirst()) {
			...
			if (mediaType == MEDIA_TYPE.VIDEO || MyApplication.flagPlayingYouTube) {
				MyLog.d(TAG, "==mediaType is VIDEO==");
...
//			timeStartCount = timeLoopChangeColor;
	...
	long time=0;
	private void startTimerVideo(final int CurrentPosition, int current) {
		try {
			//MyLog.d(TAG, "=startTimerVideo start=CurrentPosition="+CurrentPosition+"=current="+current+"=isPlaying="+mVideoView.isPlaying());
			...
			timerService.scheduleAtFixedRate(new TimerTask() {
				public void run() {
					...
					//	public void run() {		
							...
							time=t1;
					if(timerCounter>=CurrentPosition){	
								//MyLog.d(TAG, "====startTimerVideo end=timerCounter="+timerCounter+"=CurrentPosition="+getCurrentPositionVideo());
						iStartTimerVideo=false;
								stopTimer();
						if(pauseView.getPauseView())
							mVideoView.start(); 
...
//	private boolean isSentenceLeftRight = false;
	...
	private void startTimerLyricInfo(int time) {
		stopTimerLyricInfo();
		MyLog.e(TAG, "........startTimerLyricInfo.............");
								...
								public void OnReceiverInfoLyric(String resultString) {
									if(resultString == null || resultString.equals("0")){
										...
										try{
											...
											//MyLog.d(TAG, "=syncTime start=resultValue="+resultValue+"=timerPlayer="+timerPlayer);
										...
										MyLog.d(TAG, "=syncTime=CurrentPosition="+getCurrentPositionVideo()+"=timerPlayer="+timerPlayer+"=cntDelay="+cntDelay+"=iStartTimerVideo="+iStartTimerVideo+"=iPrepare="+iPrepare+"=isPlaying="+mVideoView.isPlaying());
...
//		});
		mVideoView.setOnSeekCompleteListener(new IMediaPlayer.OnSeekCompleteListener() {
			...
			public void onSeekComplete(IMediaPlayer mp) {
				// TODO Auto-generated method stub
				//if(iPrepare)iPrepare=false;
//				final int tam1=timerPlayer+cntDelay;
				isSeeking = false;
...
//				if(tam1<(getDurationVideo()-TIMEPLUSVIDEO) && (tam1>getCurrentPositionVideo()+500)){
//					//MyLog.d(TAG, "==setOnSeekCompleteListener4="+iStartTimerVideo);
//					pauseVideo();
//					if(iStartTimerVideo){	 
//						 stopTimer();
//							iStartTimerVideo=false;
//					 }					 					
//					setCurrentPositionVideo(tam1,TIMEPLUSVIDEO);
//					return;
//				}
				mVideoView.pause();				
					//mBottomLine.postDelayed(new Runnable() {
						//@Override public void run() {
							...
							//MyLog.d(TAG, "==setOnSeekCompleteListener here1="+getDurationVideo()+"=="+getCurrentPositionVideo()+"=="+timerPlayer+"=="+System.currentTimeMillis());
							int CurrentPosition=getCurrentPositionVideo();
							MyLog.d(TAG, "==setOnSeekCompleteListener here1=CurrentPosition="+CurrentPosition+"=isPlaying="+mVideoView.isPlaying()+"=timerPlayer="+timerPlayer+"=cntDelay="+cntDelay);
...
//							else {										
//								//MyLog.d(TAG, "==setOnSeekCompleteListener3="+iStartTimerVideo+"=isPlaying="+mVideoView.isPlaying());
//								
//								if(timerPlayer<(getDurationVideo()-TIMEPLUSVIDEO) && (timerPlayer>getCurrentPositionVideo()+TIMECheckPause)){
//									MyLog.d(TAG, "==setOnSeekCompleteListener4="+iStartTimerVideo);
//									pauseVideo();
//									if(iStartTimerVideo){	 
//										 stopTimer();
...
//									 }
//									 						
//									setCurrentPositionVideo(timerPlayer,TIMEPLUSVIDEO);
//									return;
//								}
//							}
						//}
