...
public class Progressbar extends View {
    ...
    private Drawable  drawBG_Slide, drawBG_Fill, drawBG_Volume_Icon;
...
//        textVolume = getResources().getString(R.string.por_volume_1);
        drawBG_Slide = getResources().getDrawable(R.drawable.line_music_1);
        drawBG_Fill = getResources().getDrawable(R.drawable.line_music_2);
        drawBG_Volume_Icon = getResources().getDrawable(R.drawable.btn_circle_line);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int mTop = h/4;
        int mBottom = h*3/4;
        int mLeft = (int) (0.02f*w);
        int mRight = (int) (w*0.998);
        rectBG_Slide.set(mLeft, mTop, mRight, mBottom);
        rectVolume.set(mLeft, mTop, mRight, mBottom);

        spaceTick = rectVolume.width() / (max_val-min_val);
        tamX = rectBG_Slide.centerX();
        int tamY = rectBG_Slide.centerY();
        height = h/3;
        width = height;

        rectVolume_Icon.set(tamX - width, tamY - height, tamX + width, tamY + height);
        rectVolume_Fill.set(rectBG_Slide.left+5*getWidth()/640, rectBG_Slide.top, rectVolume_Icon.centerX(), rectBG_Slide.bottom);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:{
                ...
                if(mX >= (rectBG_Slide.left ) && mX <= (rectBG_Slide.right ) && mY >= rectBG_Slide.top && mY <= rectBG_Slide.bottom){
                    rectVolume_Icon.set((int) (mX - rectVolume_Icon.height() / 2),rectVolume_Icon.top, (int)(mX + rectVolume_Icon.height() / 2), rectVolume_Icon.bottom);
                    rectVolume_Fill.set(rectBG_Slide.left+5*getWidth()/640, rectBG_Slide.top, rectVolume_Icon.centerX(), rectBG_Slide.bottom);
                    invalidate();
                }
            }
            ...
            case MotionEvent.ACTION_DOWN:{
                float mX = event.getX();
                float mY = event.getY();
                if(mX >= (rectTick.left ) && mX <= (rectBG_Slide.right) && mY >= rectVolume.top && mY <= rectVolume.bottom){
