...
public class FragmentSong extends FragmentBase implements
        OnMainListener, BaseLoadSong.OnLoadListener,OnBntFilterListener {
    ...
    private SongAdapter adapterSong = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        layoutShowThongBao.setOnTouchListener(new View.OnTouchListener() {
            @Override
        ...
        view.findViewById(R.id.layoutShow).setOnTouchListener(new View.OnTouchListener() {
...
//						}
                    ...
                    adapterSong = new SongAdapter(context, R.layout.touch_item_song_list,
                    ...
                    adapterSong.setOnAdapterListener(new TouchIAdapter() {
                        ...
                        public void OnShowLyric(int position, Song song) {
                            ...
                            LoadLyric loadLyric = new LoadLyric(context, (KTVMainActivity) getActivity());
...
//						StopLoadData();
                    ...
                    loadData = new vn.com.sonca.LoadSong.LoadSongFromDatabase(search, typeFragment,
                            idFragment, state1, state2, listArraySongs, context,
                            mainActivity, IdThread);
                    loadData.setOnLoadListener(FragmentSong.this);
                    loadData.execute();
...
//					}
    ...
    private vn.com.sonca.LoadSong.LoadSongFromDatabase loadData;
    ...
    private int stateScrollListView = AbsListView.OnScrollListener.SCROLL_STATE_IDLE;
    private AbsListView.OnScrollListener onScrollListener = new AbsListView.OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int first, int visible, int total) {
            mainActivity.setTotal(listView.getFirstVisiblePosition()+1,totalrec);
            if (loadData == null) {
                return;
            }else{
                int searchLength = 0;
...
//	}
    ...
    private void startLoadTotalSong(final String where, final String type, final String id, int state1, final int state2,
                                    ArrayList<Song> listSongs, final Context context , KTVMainActivity mainActivity , int IdThread){
        stopLoadTotalSong();
        ...
        timerLoadTotalSong.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                AppConfig.MEDIA_TYPE typeMedia;
                switch (state2) {
                    case TouchSearchView.VIDEO:	typeMedia = AppConfig.MEDIA_TYPE.VIDEO;		break;
                    case TouchSearchView.MIDI:	typeMedia = AppConfig.MEDIA_TYPE.MIDI;		break;
                    case TouchSearchView.TATCA:	typeMedia = AppConfig.MEDIA_TYPE.ALL;			break;
                    case TouchSearchView.ONLINE:{
                        typeMedia = AppConfig.MEDIA_TYPE.ALL;
                        flagFilterOnline = true;
                    }
                    break;
                    default: typeMedia = AppConfig.MEDIA_TYPE.ALL; break;
                }
                int count = 0;
                if (type.equals(TouchMainActivity.SINGER)) {
                    count = DBInterface.DBCountTotalSongTypeID(context, "", DBInstance.SearchType.SEARCH_SINGER, typeMedia, Integer.valueOf(id));
                } else if (type.equals(TouchMainActivity.MUSICIAN)) {
                    count = DBInterface.DBCountTotalSongTypeID(context, "", DBInstance.SearchType.SEARCH_MUSICIAN, typeMedia, Integer.valueOf(id));
                    ...
                    if(Integer.valueOf(id) == DbHelper.SongType_NewVol && (MyApplication.intSvrModel == MyApplication.SONCA_KM1
                        ...
                        count = DBInterface.DBCountTotalSongTypeID_KM(context, where, DBInstance.SearchType.SEARCH_TYPE, typeMedia, Integer.valueOf(id));
                    } else {
                        if(id.equals("" + DbHelper.SongType_China)){
                            int[] intIDs = new int[1];
                            intIDs[0] = 3; // Nhac Hoa
                            count = DBInterface.DBCountTotalSongFull(context, intIDs, where, DBInstance.SearchMode.MODE_MIXED, typeMedia);
                        } else if(id.equals("" + DbHelper.SongType_Online)){
                            count = -99;
                        } else {
                            count = DBInterface.DBCountTotalSongTypeID(context, where, DBInstance.SearchType.SEARCH_TYPE, typeMedia, Integer.valueOf(id));
                        }
                    }
                } else if (type.equals(TouchMainActivity.LANGUAGE)) {
                    count = DBInterface.DBCountTotalSongTypeID(context,where, DBInstance.SearchType.SEARCH_LANGUAGE, typeMedia, Integer.valueOf(id));
                    ...
                    if(flagFilterOnline){
                        ...
                        if(where.equals("")){
                            ...
                            count = DBInterface.DBCountTotalSong(context, intIDs, where, DBInstance.SearchMode.MODE_MIXED, typeMedia);
                        }
                    }
                ...
                if(count != -99 && MyApplication.intSvrModel == MyApplication.SONCA_SMARTK
                    ...
                    count += DBInterface.DBCountTotalSong_YouTube(context, intIDs, where,
                            DBInstance.SearchMode.MODE_MIXED, typeMedia);
                }
                ...
                if(count == -99){ // special case the loai online
                    ...
                    Cursor cursor = DBInterface.DBGetSongCursor_YouTube(context,
                            intIDs, where, DBInstance.SearchMode.MODE_MIXED, typeMedia, 0, 0);
...
//									MyLog.d("remove bot song trong youtube", song.getName() + " ");
    ...
    private Handler handlerLoadTotalSong = new Handler(){
        public void handleMessage(Message msg) {
            ...
            totalrec = total;
            mainActivity.setTotal(listView.getFirstVisiblePosition()+1,total);
        };
    };
...
//                listView.smoothScrollByOffset(5);
    ...
    private void startTimerCheckSong() {
        ...
        timerCheckSong.schedule(new FragmentSong.CheckSongTask(), 3000, 1000);
    }
