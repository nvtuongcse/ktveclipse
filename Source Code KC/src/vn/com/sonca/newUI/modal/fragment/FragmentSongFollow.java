...
import vn.com.sonca.Touch.SongFollow.TouchAdapterSongFollow;
...
public class FragmentSongFollow extends FragmentBase implements
		OnMainListener, OnLoadListener,OnBntFilterListener {
	...
	private AdapterSongFollow adapterSong = null;
	private TouchTotalView totalView;
	private LinearLayout layoutShowThongBao;
	private ItemSinger singerItemView;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.fragment_song_follow, container,
				false);
		...
		singerItemView = (ItemSinger) view.findViewById(R.id.imageSinger);

//		singerItemView.setData();
...
//		}
		...
		adapterSong = new AdapterSongFollow(context,
		...
		listView.setAdapter(adapterSong);
			//------------//
		StopLoadData();
		loadData = new LoadSongFromDatabase(search, typeFragment, idFragment, state1, state2, listArraySongs, context, mainActivity, IdThread);
		loadData.setOnLoadListener(this);
		loadData.execute();
		startLoadTotalSong(search, typeFragment, idFragment, state1, state2, listArraySongs, context, mainActivity, IdThread);
...
//                listView.smoothScrollByOffset(5);
	...
	public void OnFinishLoad() {
		if (this.isAdded() && adapterSong != null) {
			adapterSong.notifyDataSetChanged();
			if (!listArraySongs.isEmpty()){
				if(typeFragment.equals(KTVMainActivity.SINGER)) {
					Singer singer = listArraySongs.get(0).getSinger();
					singerItemView.setData(singer.getName(),"",singer.getCoverID());
				}
				else {
					Musician singer = listArraySongs.get(0).getMusician();
					singerItemView.setData(singer.getName(),"",singer.getCoverID());
				}
	...
	private void startLoadTotalSong(final String where, final String type, final String id, int state1, final int state2,
									ArrayList<Song> listSongs, final Context context , final TouchMainActivity mainActivity , int IdThread){
		stopLoadTotalSong();
