...
public class TouchPopupViewUpdateToc extends View {
	...
	private void initView(Context context) {
		...
		if(KTVNEWUI){
			drawKhung = getResources().getDrawable(R.drawable.bg_dialog);
		drawButtonActive = getResources().getDrawable(
				R.drawable.bnt_hover);
		drawButtonInactive = getResources().getDrawable(
				R.drawable.bnt);
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI) {
			...
			if(KTVNEWUI) color_01 = Color.WHITE;
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI) {
			if (popupLayout == LAYOUT_UPDATE) {
				...
				float textWidth = mainPaint.measureText(text);
				if(KTVNEWUI) mainPaint.setColor(Color.YELLOW);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT1Y,
						mainPaint);

				if(KTVNEWUI) mainPaint.setColor(color_01);
				...
				textWidth = mainPaint.measureText(text);
				if(KTVNEWUI) mainPaint.setColor(Color.YELLOW);
				if(KTVNEWUI) mainPaint.setColor(Color.parseColor("#902e00"));
				canvas.drawText(text, (KD1R + KD1L) / 2 - textWidth / 2, KT4Y,
						mainPaint);
